...
列表推导式：
格式：
1：[表达式 for 变量 in 旧列表]或者[表达式 for 变量 in 旧列表 if 条件]
2：[条件成立执行的代码 if 条件 else:条件不成立执行的代码 for 变量 in 旧列表],其中，可以将条件当做整个表达式
"""
# 过滤掉长度小于或者等于3的人名
# names = ['tom', 'lily', 'abc', 'steven', 'bob', 'ha']
# result = [name for name in names if len(name) > 3]  # 此处name实际就是返回到新列表中的元素
# print(result)
#
...
# newlist = [i[-1] for i in list] i变量其实就是旧列表种循环出来的元素，例如：[1,2,3]、[4,5,6]。。。
# print(newlist)
#
