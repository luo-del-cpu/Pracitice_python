...
"""
方法使用补充说明:
1：可以在初始化方法中传入参数，用于在创建对象时要求传参
2：可以在方法中传入参数，用于在对象调用方法时要求传参
"""


class Test:
    def __init__(self, name, age):  # 此处写了两个参数,用于定义在创建对象时，必须传递两个参数；方便对象初始化属性
        self.name = name
        self.age = age

    def eat(self, food):  # 此处穿了一个food参数，用于在对象调用方法时必须传递参数
